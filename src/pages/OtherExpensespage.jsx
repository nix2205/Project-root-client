...
// src/pages/OtherExpensesPage.jsx
import React, { useState } from "react";
import axios from "axios";
import dayjs from "dayjs";
import isSameOrBefore from "dayjs/plugin/isSameOrBefore";
...
import { monthKeysFromDate, isMonthLockedForUser } from "../utils/monthHelpers";
...
const OtherExpensesPage = () => {
  ...
  const handleSave = async () => {
    ...
    // === NEW: Check month lock on client BEFORE posting ===
    try {
      const { short: monthShort, long: monthLong } = monthKeysFromDate(formattedDate);
      const locked = isMonthLockedForUser(userInfo?.months || [], monthShort, monthLong, userInfo?.role);
      if (locked) {
        const msg = `Month ${monthShort || monthLong} has been submitted and is locked. You cannot add other expenses for this month.`;
        setMessage(msg);
        alert(msg);
        return; // early return — no network call
      }
    } catch (e) {
      // if helper fails for some reason, continue to attempt save (server will still guard)
      if (process.env.NODE_ENV === "development") console.debug("month lock check failed:", e);
    }
    // =======================================================
    ...
    try {
      ...
      // quieter: show user-friendly message; log details only in dev
      const errMsg = err?.response?.data?.msg || err?.message || "Error saving expenses";
      setMessage(errMsg);
      alert(errMsg);
      if (process.env.NODE_ENV === "development") {
        console.debug("Error saving expenses:", err);
      }
    }
  };
